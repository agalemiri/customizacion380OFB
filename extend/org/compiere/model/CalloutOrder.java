...
public class CalloutOrder extends CalloutEngine
{
	 ...
	 *  @return Error message or ""
	 */
	public String bPartnerOLD (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
	 ...
	 *  @return null or error message
	 */
	public String productB (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		Integer M_Product_ID = (Integer)value;
		if (M_Product_ID == null || M_Product_ID.intValue() == 0)
			return "";
		...
		if (Env.isSOTrx(ctx, WindowNo))
		{
			...
			if (product.isStocked())
			{
				...
				/**faaguilar OFB calculo de disponible para cotizacion proposal begin*/
				int Order_ID = Env.getContextAsInt(ctx, WindowNo, "C_Order_ID");
				MDocType doc= MDocType.get(ctx, new MOrder(ctx,Order_ID,null).getC_DocTypeTarget_ID());
				if(doc.getDocSubTypeSO().equals(MOrder.DocSubTypeSO_Proposal))
					available = MStorage.getQtyAvailableProposal
	...
	/**
	 * nuevo metodo copiado de 360 por problemas en traer termino de pago y direccion
	 * @param ctx
	 * @param WindowNo
	 * @param mTab
	 * @param mField
	...
	public String bPartner (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		try
		{
			...
			if (rs.next())
			{
				...
				{
					String loc = Env.getContext(ctx, WindowNo, Env.TAB_INFO, "C_BPartner_Location_ID");
					if (loc.length() > 1 && loc.compareTo("0") != 0)
						shipTo_ID = Integer.parseInt(loc);
				}
				if (shipTo_ID == 0)
					mTab.setValue("C_BPartner_Location_ID", null);
				...
				if (IsSOTrx)
				{
					double CreditLimit = rs.getDouble("SO_CreditLimit");
					String SOCreditStatus = rs.getString("SOCreditStatus");
					//OFB faaguilar credit check
					if(SOCreditStatus.equals("H") || SOCreditStatus.equals("S"))
					{
						mTab.fireDataStatusEEvent ("Revizar Estado del Credito", "Retenido o Detenido -Atencion-", false);	
					}
					//OFB end
					if (CreditLimit != 0)
					{
				...
				else
				{
					...
					if (s != null && s.length() != 0)
					{
						if (s.equals("B"))				//	No Cache in Non POS
							s = "P";
						/*if (IsSOTrx && (s.equals("S") || s.equals("U")))	//	No Check/Transfer for SO_Trx
							s = "P";										//  Payment Term*/ //faaguilar OFB commented
						mTab.setValue("PaymentRule", s);
					}
					//	Payment Term
					ii = new Integer(rs.getInt(IsSOTrx ? "C_PaymentTerm_ID" : "PO_PaymentTerm_ID"));
	...
	public String qtyW (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		{
			MProduct product = MProduct.get (ctx, M_Product_ID);
			if (product.isStocked() && product.get_ValueAsBoolean("CheckQty"))//faaguilar OFB add && product.get_ValueAsBoolean("CheckQty")
			{
				int M_Warehouse_ID = Env.getContextAsInt(ctx, WindowNo, "M_Warehouse_ID");
				int M_AttributeSetInstance_ID = Env.getContextAsInt(ctx, WindowNo, "M_AttributeSetInstance_ID");
	...
	public String extraDiscountPL (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		Integer M_Product_ID = (Integer)value;
		if (M_Product_ID == null || M_Product_ID.intValue() == 0)
			return "";
		...
		int ListVersion_ID=Env.getContextAsInt(ctx, Env.WINDOW_INFO, Env.TAB_INFO, "M_PriceList_Version_ID");
		String sql = "SELECT nvl(Discount2,0), nvl(Discount3,0), nvl(Discount4,0), nvl(Discount5,0)"
		           + " from M_ProductPrice where M_Product_ID="+M_Product_ID +" and M_PriceList_Version_ID="+ListVersion_ID;
		...
		try
		{
			...
			if (rs.next())
			{
				...
				mTab.setValue("Discount2", rs.getBigDecimal(1));
				mTab.setValue("Discount3", rs.getBigDecimal(2));
				mTab.setValue("Discount4", rs.getBigDecimal(3));
				mTab.setValue("Discount5", rs.getBigDecimal(4));
	...
	//faaguilar OFB Windsord
		public String productW (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
		{
			Integer M_Product_ID = (Integer)value;
			if (M_Product_ID == null || M_Product_ID.intValue() == 0)
				return "";
			...
			//faaguilar OFB PLV log begin	
			String deplog= (String)mTab.getValue("Description2");
			deplog+="|PLV:"+M_PriceList_Version_ID+"|P:"+M_Product_ID+"|PL:"+pp.getPriceList();
			mTab.setValue("Description2", deplog);
			try{
			...
			if (Env.isSOTrx(ctx, WindowNo))
			{
				...
				if (product.isStocked() && product.get_ValueAsBoolean("CheckQty"))//faaguilar OFB add && product.get_ValueAsBoolean("CheckQty")
				{
					...
					{
						BigDecimal temp = DB.getSQLValueBD(null, "select  BOMQTYAVAILABLE(M_Product_ID,?,0) from M_Product  " +
								"Where M_Product_ID=? ", wh.getM_Warehouse_ID(),M_Product_ID.intValue());
		...
		//faaguilar OFB copy value demand
		public String copy (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
		{
			if (isCalloutActive() || value == null)
				return "";
			setCalloutActive(true);
	      
	      BigDecimal Demand = (BigDecimal)mTab.getValue("Demand");
	      
	      if(Demand.compareTo(Env.ZERO)==0)
	      mTab.setValue("Demand", (BigDecimal)value); 
