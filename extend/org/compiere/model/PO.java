		...
		String acctBaseTable, String whereClause)
	{
			...
			|| acctTable.startsWith("C_BP_"))
		{
			...
			String sql = "SELECT c.ColumnName "
				...
				+ "WHERE t.TableName=? AND c.IsActive='Y' AND c.AD_Reference_Value_ID=175 ORDER BY c.ColumnName"; //faaguilar OFB change reference 25
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			try
			{
...
//				int index = p_info.getColumnIndex(columnName);
	...
	/**
	 * faaguilar OFB
	 * reseteo general de contabilidad para todos los documentos*/
	public int resetPost(int Record_ID, int Table__ID)
	{
		String sql=new String();
		sql="delete from fact_acct where record_id="+Record_ID+" and ad_table_id="+Table__ID;
