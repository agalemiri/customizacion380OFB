...
//import org.ofb.process.InOutGenerateOFB;
...
//import org.ofb.process.InOutGenerateOFB.SParameter;
...
public class MOrder extends X_C_Order implements DocAction
{
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if (   is_ValueChanged("AD_Org_ID")
			...
			for (MOrderLine line : lines) 
			{
				...
				//ininoles que no se reemplace para copesa la direccion de la linea
				if(!OFBForward.NoCopyLocationLineOrder())
				{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			if (product != null) 
			{
				if (product.isStocked())
				{
					...
					//ininoles se sobreescribe ubicación para uso en copesa
					if(OFBForward.UseOLLocatorToReserved())
					{
	...
	public boolean calculateTaxTotal()
	{
		...
		//ininoles no deben usarse lineas inactivas para el calculo de totales
		if(OFBForward.NoUseDesactiveOrderLines())
			lines = getLines(" AND IsActive = 'Y' ", " Line");
		else 
			lines = getLines();
		for (int i = 0; i < lines.length; i++)
		{
	...
	public String completeIt()
	{
		...
		if (MDocType.DOCSUBTYPESO_OnCreditOrder.equals(DocSubTypeSO)		//	(W)illCall(I)nvoice
			...
			|| MDocType.DOCSUBTYPESO_PrepayOrder.equals(DocSubTypeSO)) 
		{
			//ininoles no se setara como forzado sino como disponible para artilec
			...
			if(OFBForward.UseGenInOutARTILEC())
			{
				//shipment = generate(this);
				//ininoles nuevos metodos para generacion de despachos
				shipment = generate(this);
			}
			else
	...
	//	shipment.setDateAcct(getDateAcct());
		...
		if(OFBForward.NoCompleteMinOutWOrderArtilec())
			;//no se completa recibo si existe registro OFBForward
		else
	...
	private MInvoice createInvoice (MDocType dt, MInOut shipment, Timestamp invoiceDate)
	{
		...
		{
			//ininoles nose seteara nunca como inmediato la regla de facturacion
			//para artilec
			if(OFBForward.SetDeliveryRuleOrderArtilec())
			{
				setInvoiceRule(INVOICERULE_AfterDelivery);
	...
	//ininoles nuevos metodos sacados de InOutGenerate.java por problemas en las ubicaciones 
	...
	{
		boolean	p_IsUnconfirmedInOut = false;
		//	New Header different Shipper, Shipment Location
		Timestamp minGuaranteeDate = order.getDateOrdered();
	...
	{
			//int	m_lastC_BPartner_Location_ID = -1;
			...
			/*if (m_shipment == null)
			{
				m_shipment = new MInOut (order, 0, Env.getContextAsDate(getCtx(), "#Date"));
			  ...
			  Timestamp minGuaranteeDate, boolean FiFo)
	{
		...
		if (m_lastStorages == null)
		{
			...
			/*m_lastStorages = getWarehouse(getCtx(), 
					M_Warehouse_ID, M_Product_ID, M_AttributeSetInstance_ID,
	...
	public MStorage[] getWarehouse (Properties ctx, int M_Warehouse_ID, 
			...
			boolean FiFo, boolean positiveOnly, int M_Locator_ID, String trxName)
		{
			...
			sql += "ORDER BY s.QtyOnHand DESC,l.PriorityNo DESC, M_AttributeSetInstance_ID";
			if (!FiFo)
				sql += " DESC";
	...
	private MInOut generate (MOrder order)
	{
		...
		if (order.getDatePromised() != null)
			where += " AND (TRUNC(DatePromised)<=" + DB.TO_DATE(order.getDatePromised(), true)
				+ " OR DatePromised IS NULL)";		
		MOrderLine[] lines = order.getLines (where, "ORDER BY C_BPartner_Location_ID, M_Product_ID");
		exploreLines(lines,completeOrder,completeLine,available, order);
...
//			Stored Product
			...
			Integer[] storages, boolean force)
	{
		...
		if (m_shipment == null)
		{
			...
			m_shipment = new MInOut (order, order.getC_DocType().getC_DocTypeShipment_ID(), DateUtils.today());
			m_shipment.setM_Warehouse_ID(orderLine.getM_Warehouse_ID());	//	sets Org too
			if (order.getC_BPartner_ID() != orderLine.getC_BPartner_ID())
				m_shipment.setC_BPartner_ID(orderLine.getC_BPartner_ID());
			if (order.getC_BPartner_Location_ID() != orderLine.getC_BPartner_Location_ID())
