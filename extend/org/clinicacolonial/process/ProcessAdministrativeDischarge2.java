...
public class ProcessAdministrativeDischarge2 extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		if (p_AdmDis_ID > 0)
		{
			...
			if(adm.isProcessed() == false && p_Action.compareTo("AL") == 0)
			{
				...
				*/
				...
				DB.executeUpdate("UPDATE CC_Operation set processed='Y' where cc_hospitalization_id = "+adm.getCC_Hospitalization_ID(),get_TrxName());
				DB.executeUpdate("UPDATE CC_Evaluation set processed2='Y' where cc_hospitalization_id = "+adm.getCC_Hospitalization_ID(),get_TrxName());
				DB.executeUpdate("UPDATE CC_MedicalOrder set processed2='Y' where cc_hospitalization_id = "+adm.getCC_Hospitalization_ID(),get_TrxName());
				DB.executeUpdate("UPDATE CC_CIE10 set processed2='Y' where c_bpartner_id = "+partnerhosp,get_TrxName());
				DB.executeUpdate("UPDATE CC_MedicalIndications set processed2='Y' where c_bpartner_id = "+partnerhosp,get_TrxName());
				DB.executeUpdate("UPDATE CC_ShiftChange set processed2='Y' where cc_hospitalization_id = "+adm.getCC_Hospitalization_ID(),get_TrxName());
				DB.executeUpdate("UPDATE CC_Epicrisis set processed2='Y' where cc_hospitalization_id = "+adm.getCC_Hospitalization_ID(),get_TrxName());
				DB.executeUpdate("UPDATE CC_Other_Indications set processed2='Y' where c_bpartner_id = "+partnerhosp,get_TrxName());
				log.config("lalala");
				//adm.setProcessed(true);
				adm.set_CustomColumn("Processed2", true);
